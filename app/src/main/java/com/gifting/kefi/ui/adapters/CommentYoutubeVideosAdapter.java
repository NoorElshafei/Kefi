...
public class CommentYoutubeVideosAdapter extends RecyclerView.Adapter<CommentYoutubeVideosAdapter.ViewHolder> {
    ...
    public CommentYoutubeVideosAdapter(Context context) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if (getItemViewType(position) == ITEM) {
            ...
            holder.binding1.nameShortCut.setText(getShortCutOfName(videoCommentModelArrayList.get(position).getUserName()));
            long millisecond = videoCommentModelArrayList.get(position).getCommentDate();
            String dateString = new SimpleDateFormat("dd MMM yyyy").format(new Date(millisecond));
